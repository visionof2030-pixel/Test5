...
export function openAnalytics() {
    ...
    setTimeout(() => {
        ...
        if (select.value) { updatePlayerAnalytics(); }
    }, 300);
}
...
export function populatePlayerAnalyticsSelect() {
    ...
    if (players.includes(currentUser)) { select.value = currentUser; }
}
...
export function updatePlayerAnalytics() {
    ...
    if (!playerName) { detailsDiv.style.display = 'none'; return; }
    const predictions = state.predictions || [];
    const games = state.previousGamesData || [];
    const playerPreds = predictions.filter(p => p.user_name === playerName);
    if (playerPreds.length === 0) { detailsDiv.innerHTML = `<div class="empty-state"><span class="icon">📭</span> لا توجد توقعات لهذا اللاعب</div>`; detailsDiv.style.display = 'block'; return; }
    let total = playerPreds.length, correct = 0, wrong = 0, points = 0;
    const sortedPreds = [...playerPreds].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
    for (let p of sortedPreds) { const status = getPredictionStatus(p); if (status.status === 'correct') { correct++; points++; } else if (status.status === 'wrong') { wrong++; } }
    const recent = sortedPreds.slice(-5); let recentCorrect = 0; for (let p of recent) { const status = getPredictionStatus(p); if (status.status === 'correct') recentCorrect++; }
    const recentAcc = recent.length > 0 ? (recentCorrect / recent.length) * 100 : 0;
    const overallAcc = total > 0 ? (correct / total) * 100 : 0;
    const allStats = getAllPlayersStats(); const totalPlayers = Object.keys(allStats).length;
    ...
    const futureRanks = Object.values(allStats).map(p => { const pAvg = p.total > 0 ? p.points / p.total : 0; const pFuture = p.points + (pAvg * remainingMatches); return { name: p.name, futurePoints: pFuture }; }).sort((a, b) => b.futurePoints - a.futurePoints);
...
export function renderMatchAnalytics() {
    ...
    const predictions = state.predictions || [];
    const games = state.previousGamesData || [];
    if (!predictions.length || !games.length) { grid.innerHTML = `<div class="empty-state"><span class="icon">📊</span> لا توجد بيانات كافية</div>`; return; }
    const matchStats = {};
    ...
    let html = `
    <div class="match-list"><div class="list-title">✅ أكثر المباريات توقعاً صحيحاً</div>
    ${sortedCorrect.length === 0 ? '<div class="empty-state" style="padding:10px;">لا توجد بيانات</div>' : ''}
    ${sortedCorrect.map(([match, stats]) => `<div class="match-item"><span class="teams">${match}</span><span class="count correct">${stats.correct} صحيح</span></div>`).join('')}
    </div>
    <div class="match-list"><div class="list-title">❌ أكثر المباريات توقعاً خاطئاً</div>
    ${sortedWrong.length === 0 ? '<div class="empty-state" style="padding:10px;">لا توجد بيانات</div>' : ''}
    ${sortedWrong.map(([match, stats]) => `<div class="match-item"><span class="teams">${match}</span><span class="count wrong">${stats.wrong} خاطئ</span></div>`).join('')}
    </div>
  `;
    grid.innerHTML = html;
}
...
export function generateAnalyticsCharts() {
    ...
    if (!predictions.length || !games.length) {
      ...
      <div style="text-align:center;margin-top:12px;"><button class="tab-btn" onclick="document.getElementById('analyticsModal').classList.remove('active');document.body.style.overflow='';" style="background:rgba(240,180,41,0.08);border-color:rgba(240,180,41,0.2);color:var(--gold-light);">إغلاق</button></div>
    `;
        return;
    }
    const playerPredCount = {};
    for (let p of predictions) { if (!playerPredCount[p.user_name]) playerPredCount[p.user_name] = 0; playerPredCount[p.user_name]++; }
    ...
    for (let p of predictions) {
        ...
        if (parts.length < 3) continue;
        const team1 = parts[1], team2 = parts[2];
        const match = games.find(g => (g.homeAr === team1 && g.awayAr === team2) || (g.homeAr === team2 && g.awayAr === team1));
        if (!match) continue;
    ...
    for (let [user, preds] of Object.entries(userPoints)) {
        let total = 0;
        cumulativePoints[user] = preds.map(p => { if (p.correct) total++; return total; });
    }
    const topUsers = Object.entries(cumulativePoints).sort((a, b) => b[1][b[1].length - 1] - a[1][a[1].length - 1]).slice(0, 5);
    ...
    const popularMatches = Object.entries(matchPredCount).sort((a, b) => b[1] - a[1]).slice(0, 8);
    ...
    const topAcc = Object.entries(accuracyStats).filter(([_, v]) => v.total >= 2).sort((a, b) => (b[1].correct / b[1].total) - (a[1].correct / a[1].total)).slice(0, 5);
    ...
    chartInstancesLocal.predDist = new Chart(ctx1, { type: 'bar', data: { labels: predLabels, datasets: [{ label: 'عدد التوقعات', data: predData, backgroundColor: 'rgba(240, 180, 41, 0.7)', borderColor: 'rgba(240, 180, 41, 1)', borderWidth: 1 }] }, options: { responsive: true, plugins: { legend: { labels: { color: '#8899bb', font: { family: 'Cairo' } } } }, scales: { x: { ticks: { color: '#8899bb', font: { family: 'Cairo', size: 9 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#8899bb' }, grid: { color: 'rgba(255,255,255,0.04)' }, beginAtZero: true } } } });
    const ctx2 = document.getElementById('chartPoints').getContext('2d');
    const colors = ['#f0b429', '#5dade2', '#2ecc71', '#e74c3c', '#9b59b6'];
    const datasets = topUsers.map((user, idx) => ({ label: user[0], data: user[1], borderColor: colors[idx % colors.length], backgroundColor: colors[idx % colors.length] + '33', fill: true, tension: 0.3, pointRadius: 2 }));
    chartInstancesLocal.points = new Chart(ctx2, { type: 'line', data: { labels: pointLabels, datasets: datasets }, options: { responsive: true, plugins: { legend: { labels: { color: '#8899bb', font: { family: 'Cairo', size: 10 } } } }, scales: { x: { ticks: { color: '#8899bb', font: { family: 'Cairo', size: 8 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#8899bb' }, grid: { color: 'rgba(255,255,255,0.04)' }, beginAtZero: true } } } });
    const ctx3 = document.getElementById('chartPopular').getContext('2d');
    chartInstancesLocal.pop = new Chart(ctx3, { type: 'bar', data: { labels: popLabels, datasets: [{ label: 'عدد التوقعات', data: popData, backgroundColor: 'rgba(240, 180, 41, 0.6)', borderColor: 'rgba(240, 180, 41, 1)', borderWidth: 1 }] }, options: { responsive: true, plugins: { legend: { labels: { color: '#8899bb', font: { family: 'Cairo' } } } }, scales: { x: { ticks: { color: '#8899bb', font: { family: 'Cairo', size: 9 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#8899bb' }, grid: { color: 'rgba(255,255,255,0.04)' }, beginAtZero: true } } } });
    const ctx4 = document.getElementById('chartAccuracy').getContext('2d');
    chartInstancesLocal.acc = new Chart(ctx4, { type: 'bar', data: { labels: accLabels, datasets: [{ label: 'نسبة التوقعات الصحيحة (%)', data: accData, backgroundColor: 'rgba(46, 204, 113, 0.7)', borderColor: 'rgba(46, 204, 113, 1)', borderWidth: 1 }] }, options: { responsive: true, plugins: { legend: { labels: { color: '#8899bb', font: { family: 'Cairo' } } } }, scales: { x: { ticks: { color: '#8899bb', font: { family: 'Cairo', size: 9 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#8899bb' }, grid: { color: 'rgba(255,255,255,0.04)' }, beginAtZero: true, max: 100 } } } });
    setChartInstancesLocal(chartInstancesLocal);
    document.getElementById('analyticsContent').innerHTML = `
    <div class="analytics-grid">
      <div class="chart-box full-width"><div class="chart-title">📊 أكثر اللاعبين توقعاً</div><canvas id="chartPlayerDistribution"></canvas></div>
      <div class="chart-box full-width"><div class="chart-title">📈 تطور نقاط أفضل 5 لاعبين</div><canvas id="chartPoints"></canvas></div>
      <div class="chart-box full-width"><div class="chart-title">🔥 أكثر المباريات توقعاً</div><canvas id="chartPopular"></canvas></div>
      <div class="chart-box full-width"><div class="chart-title">🎯 نسبة التوقعات الصحيحة (أفضل 5)</div><canvas id="chartAccuracy"></canvas></div>
    </div>
    <div class="player-analytics-section" id="playerAnalyticsSection">
      <div class="select-player"><select id="playerAnalyticsSelect" onchange="updatePlayerAnalytics()"><option value="">-- اختر لاعباً --</option></select></div>
      <div id="playerAnalyticsDetails" class="player-analytics-details" style="display:none;"></div>
    </div>
    <div class="match-analytics-section" id="matchAnalyticsSection"><div class="match-analytics-grid" id="matchAnalyticsGrid"></div></div>
  ...
  `;
    setTimeout(() => {
        ...
        if (chartInstancesLocal.pop) chartInstancesLocal.pop.destroy();
        if (chartInstancesLocal.acc) chartInstancesLocal.acc.destroy();
        chartInstancesLocal.predDist = new Chart(ctx1b, { type: 'bar', data: { labels: predLabels, datasets: [{ label: 'عدد التوقعات', data: predData, backgroundColor: 'rgba(240, 180, 41, 0.7)', borderColor: 'rgba(240, 180, 41, 1)', borderWidth: 1 }] }, options: { responsive: true, plugins: { legend: { labels: { color: '#8899bb', font: { family: 'Cairo' } } } }, scales: { x: { ticks: { color: '#8899bb', font: { family: 'Cairo', size: 9 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#8899bb' }, grid: { color: 'rgba(255,255,255,0.04)' }, beginAtZero: true } } } });
        chartInstancesLocal.points = new Chart(ctx2b, { type: 'line', data: { labels: pointLabels, datasets: datasets }, options: { responsive: true, plugins: { legend: { labels: { color: '#8899bb', font: { family: 'Cairo', size: 10 } } } }, scales: { x: { ticks: { color: '#8899bb', font: { family: 'Cairo', size: 8 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#8899bb' }, grid: { color: 'rgba(255,255,255,0.04)' }, beginAtZero: true } } } });
        chartInstancesLocal.pop = new Chart(ctx3b, { type: 'bar', data: { labels: popLabels, datasets: [{ label: 'عدد التوقعات', data: popData, backgroundColor: 'rgba(240, 180, 41, 0.6)', borderColor: 'rgba(240, 180, 41, 1)', borderWidth: 1 }] }, options: { responsive: true, plugins: { legend: { labels: { color: '#8899bb', font: { family: 'Cairo' } } } }, scales: { x: { ticks: { color: '#8899bb', font: { family: 'Cairo', size: 9 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#8899bb' }, grid: { color: 'rgba(255,255,255,0.04)' }, beginAtZero: true } } } });
        chartInstancesLocal.acc = new Chart(ctx4b, { type: 'bar', data: { labels: accLabels, datasets: [{ label: 'نسبة التوقعات الصحيحة (%)', data: accData, backgroundColor: 'rgba(46, 204, 113, 0.7)', borderColor: 'rgba(46, 204, 113, 1)', borderWidth: 1 }] }, options: { responsive: true, plugins: { legend: { labels: { color: '#8899bb', font: { family: 'Cairo' } } } }, scales: { x: { ticks: { color: '#8899bb', font: { family: 'Cairo', size: 9 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#8899bb' }, grid: { color: 'rgba(255,255,255,0.04)' }, beginAtZero: true, max: 100 } } } });
        populatePlayerAnalyticsSelect();
        renderMatchAnalytics();
        const select = document.getElementById('playerAnalyticsSelect');
        if (select.value) { updatePlayerAnalytics(); }
